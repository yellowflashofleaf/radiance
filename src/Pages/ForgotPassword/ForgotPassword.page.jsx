...
function ForgotPasswordPage() {
  ...
  async function handleGetOTP(e) {
    ...
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_API_URL}auth/forgot-password/get-otp`,
        { email, captcha }
      );
    } catch (e) {
    } finally {
  ...
  async function handleCheckOTP(e) {
    ...
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_API_URL}auth/forgot-password/change-password`,
        {
          email,
          otp,
          password,
          ...
          <CardActions className="text-center" style={{ width: "100%" }}>
            {isOTPSent && (
              <button
                className={
                  disabled1
                    ? "event-links otp-links-disabled mt-3"
                    : "event-links event-links-active mt-3"
                }
                disabled={disabled1}
                onClick={handleCheckOTP}
              >
                Change Password {loading2 && <CircularProgress size={20} />}
              </button>
            )}
            {!isOTPSent && (
              <button
              ...
              >
                Get OTP {loading1 && <CircularProgress size={20} />}
              </button>
            )}
          </CardActions>
        </Card>
